...
func (s *AuthService) Register(c *fiber.Ctx) error {
	...
	createdID, err := s.users.Create(ctx, model.User{
	...
	if err != nil {
		// Just a simple duplicate check representation since we use generic ErrDuplicate or return raw error.
		// In a real app we'd map Postgres error codes.
	...
	return helper.Created(c, "pendaftaran berhasil", fiber.Map{"id": createdID}, "/api/v1/users/"+strconv.Itoa(createdID))
}
